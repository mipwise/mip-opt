...
for i, t in x_keys:
    if t == T[0]:
        mdl.addConstraint((pl[i]*z[i, t] + x[i, t]) - p[i] <= ru[i]*z[i, t-1] + su[i]*y[i, t],
                          name=f'C9_{i}_{t}')
    else:
        mdl.addConstraint((pl[i]*z[i, t] + x[i, t]) - (pl[i]*z[i, t-1] + x[i, t-1]) <= ru[i]*z[i, t-1] + su[i]*y[i, t],
                          name=f'C9_{i}_{t}')

# C10) Production of Generator 𝑖 must not ramp down more than 𝑠𝑑𝑖 when it's shutting down or more than 𝑟𝑑𝑖 when it's on:
for i, t in x_keys:
    if t == T[0]:
        mdl.addConstraint(p[i] - (pl[i] * z[i, t] + x[i, t]) <= rd[i] * z[i, t] + sd[i] * w[i, t], name=f'C10_{i}_{t}')
    else:
        mdl.addConstraint((pl[i] * z[i, t-1] + x[i, t-1]) - (pl[i] * z[i, t] + x[i, t])
                          <= rd[i] * z[i, t] + sd[i] * w[i, t], name=f'C10_{i}_{t}')
