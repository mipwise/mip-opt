...
for i, t in x_keys:
    if t == T[0]:
        mdl.addConstraint(x[i, t] - p[i] <= su[i] + (pu[i] - su[i]) * (1 - y[i, t]), name=f'SR1_{i}_{t}')
        mdl.addConstraint(p[i] - x[i, t] <= sd[i] + (pu[i] - sd[i]) * (1 - w[i, t]), name=f'SR2_{i}_{t}')
    else:
        mdl.addConstraint(x[i, t] - x[i, t-1] <= su[i] + (pu[i] - su[i]) * (1 - y[i, t]), name=f'SR1_{i}_{t}')
        mdl.addConstraint(x[i, t-1] - x[i, t] <= sd[i] + (pu[i] - sd[i]) * (1 - w[i, t]), name=f'SR2_{i}_{t}')
